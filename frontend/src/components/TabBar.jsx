...
export default function TabBar({ onStart }) {
  ...
  const Tab = ({ k, icon, to, label }) => (
    <button aria-current={on(k) ? 'page' : undefined} className={on(k) ? 'on' : ''} onClick={() => nav(to)}>
      <Icon name={icon} /><span>{label}</span>
    </button>
  )

  return (
    <nav id="tabbar" aria-label={t('Main navigation')}>
      ...
      <button aria-current={cur === 'workout' ? 'page' : undefined} className={'start' + (S.active ? ' rec' : '') + (S.active && cur === 'workout' ? ' on' : '')} onClick={startWorkout}>
